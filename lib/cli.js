...
`
	...
	migratedocs:{
		desc: `--------------------------
Migrates all the existing files and folders from a specified local path to a target repository with a target
node. The command is self prompting and does not require any flags.
Start with by typing: 'ims migratedocs', system will prompt you to provide promts to fill in the required 
parameters.
`
	}
};
